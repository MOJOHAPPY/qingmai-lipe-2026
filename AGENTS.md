...
- 云端同步防覆盖保护（2026-08-26）：为解决「空/默认数据设备把云端整份覆盖导致其他手机数据被冲」的隐患——新增本机 `SYNC.hasReal`（持久化 tripSyncMeta）标记「本设备是否有过真实修改」，markDirty 置真、播种路径（expenses 已有 SEEDING 守卫，reminders 播种新增守卫）与云端下载不置真；doSync 改为：云端空→上传创建；本地新且有真实修改→上传；本地新但无真实修改→不覆盖云端、改为镜像云端；云端新→下载。同步面板新增「⬆️ 强制上传 / ⬇️ 强制下载」手动方向按钮（绕过自动判断）；新设备接入默认只下载不上传。verify-app.mjs 新增独立上下文防覆盖用例（pristine 不上传 / hasReal=false 不覆盖 / 真实修改上传 / 强制按钮方向）全部通过。
